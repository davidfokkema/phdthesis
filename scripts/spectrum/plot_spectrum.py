...
def main():
    leap = np.genfromtxt('crspec/leap-prot.dat', delimiter=',',
                         usecols=(0, 1), names=['E', 'F'])
    leap['E'] *= 1e6
    leap['F'] *= 1e3 * 2

    proton = read_data('crspec/proton', 1, 0)
    akeno_new_lo = read_data('crspec/akeno-new-lo', 0, 1)
    flys_eye = read_data('crspec/fe-new', 0, 1)
    yakutsk = read_data('crspec/yakustk', 1, 0)
    haverah = read_data('crspec/haverah', 1, 0)
